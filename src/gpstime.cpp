 ...
 */
GpsTime::GpsTime(const DateTime * const t)
{
...
GpsTime::GpsTime(const GpsTime * const g, const double dt)
{
	week = g->week;
	sec = g->sec + dt;

	sec = round(g->sec*1000.0)/1000.0; // Avoid rounding error
...
double GpsTime::Sub(const GpsTime * const g)
{
	double dt;
...
DateTime::DateTime(const GpsTime * const g)
{
